    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['needdo:needdo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['needdo:needdo:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="needdoList" @selection-change="handleSelectionChange">
...
<script>
import { listNeeddo, getNeeddo, delNeeddo, addNeeddo, updateNeeddo } from "@/api/needdo/needdo";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listNeeddo(this.queryParams).then(response => {
        this.needdoList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateNeeddo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addNeeddo(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除我的待办编号为"' + ids + '"的数据项？').then(function() {
        return delNeeddo(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('needdo/needdo/export', {
        ...this.queryParams
      }, `needdo_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
